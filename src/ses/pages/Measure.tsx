    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="px-4 space-y-5 pb-8">
        <section className="ses-card space-y-3">
          ...
          {kayit && <div className="rounded-2xl bg-emerald-50 dark:bg-[#1f2e22] text-emerald-700 dark:text-emerald-300 text-[16px] p-3 text-center">Kaydedildi ✔</div>}
        </section>
              ...
              {gecmis.map((r) => (
                <li key={r.id} className="py-2 flex items-center justify-between text-[16px]">
                  ...
                  <span className="text-right">
                    ...
                    {r.ac && <span className="block text-[13px] text-slate-700 dark:text-[#d8c8bf] tabular-nums">HNR {String(r.ac.hnr).replace('.', ',')} dB · jitter %{String(r.ac.jitter).replace('.', ',')} · shimmer %{String(r.ac.shimmer).replace('.', ',')}</span>}
                  </span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-[15px] text-slate-700 dark:text-[#d8c8bf] mt-1">Henüz ölçüm yok. İlk ölçümünü yap ve kaydet.</p>
          )}
        </section>
