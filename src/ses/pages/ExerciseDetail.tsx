  ...
  if (!e) {
    return (
      <div>
        ...
        <p className="px-5 text-slate-500">Bulunamadı.</p>
      </div>
    )
  }
      ...
      <div className="px-4 space-y-4 pb-6">
        <section className="ses-card">
          <div className="flex items-center gap-3">
            <span className="w-14 h-14 rounded-2xl bg-ses-50 dark:bg-[#352820] grid place-items-center text-[30px]">{e.emoji}</span>
        ...
        <section className="ses-card">
          ...
          <p className="text-[15px] text-slate-700 dark:text-[#d8c8bf]">{e.why}</p>
        </section>
        ...
        <section className="ses-card">
          ...
          <div className="mt-3 rounded-2xl bg-ses-50 dark:bg-[#352820] p-3 text-center">
            <div className="text-[12px] text-ses-700 dark:text-ses-300 font-semibold uppercase tracking-[0.08em]">Söylenecek</div>
            <div className="text-[24px] font-bold text-slate-900 dark:text-[#f5ece4]">{Array.isArray(e.cue) ? e.cue.join(' · ') : e.cue}</div>
          </div>
        </section>
        ...
        {e.caution && (
          <section className="ses-card bg-amber-50/70 dark:bg-[#2b2418]">
            ...
            <p className="text-[15px] text-slate-700 dark:text-[#d8c8bf]">{e.caution}</p>
          </section>
        )}

        <button className="ses-btn-primary w-full min-h-[62px] text-[19px]" onClick={() => navigate(`/seans?tek=${e.id}`)}>
          ▶ Yalnızca bunu yap
        </button>
