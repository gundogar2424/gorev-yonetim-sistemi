      ...
      <div className="px-4 space-y-5 pb-6">
        <section className="ses-card">
          <div className="flex items-center gap-3">
            <span className="w-14 h-14 rounded-2xl bg-ses-50 dark:bg-[#352820] grid place-items-center text-[32px]">{e.emoji}</span>
            <p className="text-[18px] text-slate-700 dark:text-[#d8c8bf]">{e.short}</p>
          </div>
        ...
        <section className="ses-card">
          ...
          <p className="text-[17px] text-slate-700 dark:text-[#d8c8bf]">{e.why}</p>
        </section>
        ...
        <section className="ses-card">
          ...
          <div className="mt-3 rounded-2xl bg-ses-50 dark:bg-[#352820] p-3 text-center">
            <div className="text-[14px] text-ses-700 dark:text-ses-300 font-semibold uppercase tracking-[0.08em]">Söylenecek</div>
            <div className="text-[26px] font-bold text-slate-900 dark:text-[#f5ece4]">{Array.isArray(e.cue) ? e.cue.join(' · ') : e.cue}</div>
          </div>
        </section>
        ...
        {e.caution && (
          <section className="ses-card bg-amber-50/70 dark:bg-[#2b2418]">
            ...
            <p className="text-[17px] text-slate-700 dark:text-[#d8c8bf]">{e.caution}</p>
          </section>
        )}

        {(e.clip || e.video) && (
          <section className="ses-card space-y-3">
            <h3 className="ses-label">Nasıl yapılır (video)</h3>
            {e.clip && <ClipPlayer src={e.clip} title="Kısa gösterim (internetsiz çalışır)" />}
            {e.video && <VideoEmbed video={e.video} immediate={!e.clip} />}
          </section>
        )}
