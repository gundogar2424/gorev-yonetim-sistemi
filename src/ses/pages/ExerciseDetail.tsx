  ...
  if (!e) {
    return (
      <div>
        ...
        <p className="px-5 text-sesui-body">Bulunamadı.</p>
      </div>
    )
  }
    ...
    <div>
      ...
      <div className="px-4 space-y-5 pb-6">
        ...
        <section className="ses-card">
          ...
          <p className="text-[16px] text-sesui-body dark:text-sesui-dbody">{e.why}</p>
        </section>
        ...
        <section className="ses-card">
          ...
          <div className="mt-3 rounded-[10px] bg-ses-50 dark:bg-sesui-dsoft p-3 text-center">
            <div className="text-[13px] text-ses-700 dark:text-ses-300 font-semibold uppercase tracking-[0.08em]">Söylenecek</div>
            <div className="text-[22px] font-bold text-sesui-text dark:text-sesui-dtext">{Array.isArray(e.cue) ? e.cue.join(' · ') : e.cue}</div>
          </div>
        </section>

        {e.caution && (
          <section className="ses-card bg-amber-50/70 dark:bg-[#2b2418]">
            <h3 className="ses-label mb-1 flex items-center gap-1.5">
              <Icon name="alert" size={14} />
              Dikkat
            </h3>
            <p className="text-[16px] text-sesui-body dark:text-sesui-dbody">{e.caution}</p>
          </section>
        )}
