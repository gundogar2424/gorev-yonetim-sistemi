                ...
                {done.map((d, i) => {
                  ...
                  return (
                    <li key={i} className="py-2 flex items-center justify-between text-[15px]">
                      ...
                      <span className="text-slate-500 dark:text-[#a3908a]">{d.mpt ? `${d.mpt.toFixed(1).replace('.', ',')} sn` : `${d.reps} ×`}</span>
                    </li>
                  )
                })}
              </ul>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-4 space-y-4 pb-8 flex-1 flex flex-col">
          <section className="ses-card ses-pop">
            ...
            {ex.caution && <p className="mt-3 rounded-2xl bg-amber-50 dark:bg-[#2b2418] p-3 text-[14px] text-amber-800 dark:text-amber-200">⚠️ {ex.caution}</p>}
  ...
  if (phase === 'mpt') {
    return (
      <div className="flex-1 flex flex-col">
        ...
        <div className="px-4 space-y-3 pb-8 flex-1 flex flex-col">
          <section className="ses-card">
            <MptMeter key={mptAttempt} onResult={mptSonuc} attempt={mptAttempt} attempts={n} compact />
          </section>
          {mptBest > 0 && <div className="text-center text-[15px] text-slate-500 dark:text-[#a3908a]">Bu seansta en iyi: {mptBest.toFixed(1).replace('.', ',')} sn</div>}
          <div className="mt-auto space-y-2">
            <button className="ses-btn-primary w-full min-h-[62px] text-[19px]" onClick={mptSonraki} disabled={!mptResult}>
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="px-4 pb-8 flex-1 flex flex-col">
        <div className={`flex-1 flex flex-col items-center justify-center rounded-3xl p-6 text-center transition-colors ${isGo ? 'bg-ses-600 text-white' : isReady ? 'bg-ses-50 dark:bg-[#352820]' : 'bg-white dark:bg-[#261d16]'}`}>
          <div className={`text-[14px] font-semibold uppercase tracking-[0.1em] ${isGo ? 'text-white/80' : 'text-ses-700 dark:text-ses-300'}`}>{baslik}</div>
          <div className={`font-bold leading-none tabular-nums my-4 ${isGo ? 'text-[96px] ses-pulse' : 'text-[80px] text-slate-900 dark:text-[#f5ece4]'}`}>{Math.ceil(left)}</div>
          <div className={`text-[30px] font-bold leading-tight ${isGo ? 'text-white' : 'text-slate-800 dark:text-[#f5ece4]'}`}>
            {isReady ? (ex.mode === 'sure' ? 'Nefes al…' : 'Nefes al…') : isGo ? cue : 'Gevşe, nefes al'}
          </div>
          {phase === 'rest' && <div className="text-[15px] text-slate-500 dark:text-[#a3908a] mt-2">Sonraki: {Array.isArray(ex.cue) ? ex.cue[(rep + 1) % ex.cue.length] : ex.cue}</div>}
          <div className={`w-full h-2.5 rounded-full mt-6 overflow-hidden ${isGo ? 'bg-white/25' : 'bg-slate-200 dark:bg-[#4a3a30]'}`}>
            <div className={`h-full rounded-full transition-[width] duration-100 ${isGo ? 'bg-white' : 'bg-ses-500'}`} style={{ width: `${pct}%` }} />
          </div>
          {/* tekrar noktalari */}
          <div className="flex flex-wrap justify-center gap-1.5 mt-4 max-w-[260px]">
            {Array.from({ length: n }).map((_, i) => (
              <span key={i} className={`w-2.5 h-2.5 rounded-full ${i < rep || (i === rep && phase === 'rest') ? (isGo ? 'bg-white' : 'bg-ses-600') : i === rep ? (isGo ? 'bg-white/70' : 'bg-ses-300') : isGo ? 'bg-white/25' : 'bg-slate-200 dark:bg-[#4a3a30]'}`} />
            ))}
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 mt-3">
          <button className="ses-btn-ghost" onClick={duraklat}>
            {paused ? '▶ Devam' : '⏸ Dur'}
          </button>
          <button className="ses-btn-ghost" onClick={ilerle}>
            İleri ⏭
          </button>
          <button className="ses-btn-ghost" onClick={atla}>
            Atla
          </button>
        </div>
      </div>
