  ...
  return (
    <div>
      ...
      <div className="px-4 space-y-4">
              ...
              {EXERCISES.filter((e) => e.group === g).map((e) => {
                ...
                return (
                  <Link key={e.id} to={`/egzersiz/${e.id}`} className={`ses-card flex items-center gap-3 active:scale-[0.99] transition ${kapali ? 'opacity-55' : ''}`}>
                    <span className="w-12 h-12 rounded-2xl bg-ses-50 dark:bg-[#352820] grid place-items-center text-[26px] flex-shrink-0">{e.emoji}</span>
                    ...
                    <svg viewBox="0 0 24 24" className="w-5 h-5 text-slate-300 dark:text-[#a3908a]" fill="none" stroke="currentColor" strokeWidth={2.2}>
        ...
        <p className="text-[13px] text-slate-500 dark:text-[#a3908a] px-1 pb-2">
