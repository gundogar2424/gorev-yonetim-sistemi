...
export async function getFeedback(s: Session, history: Session[]): Promise<Feedback> {
  ...
  const prev = history
    ...
    .map((h) => {
      ...
      return `${h.t.slice(0, 10)} ${sessionKind(h) === 'diksiyon' ? 'diksiyon' : 'addüksiyon'}: ${h.done.length} egzersiz${acc != null ? `, doğruluk %${acc}` : ''}${mpt ? `, MPT ${mpt} sn` : ''}${typeof h.rating === 'number' ? `, öz değ. ${h.rating}/5` : ''}${top.length ? `, yutulan: ${top.join(', ')}` : ''}`
    })
