...
export default function AcousticCard({ ac, compact }: { ac: AcousticResult; compact?: boolean }) {
  ...
  return (
    <div className={`rounded-[10px] bg-sesui-soft dark:bg-sesui-dsoft ${compact ? 'p-3' : 'p-4'}`}>
      <div className="ses-label mb-2">Ses kalitesi (akustik)</div>
      <ul className="divide-y divide-slate-200/70 dark:divide-sesui-dline">
        {items.map((it) => (
          <li key={it.ad} className="py-1.5 flex items-center gap-2">
            <span className="flex-1 min-w-0">
              <span className="block text-[15px] text-sesui-body dark:text-sesui-dtext">{it.ad}</span>
              <span className="block text-[13px] text-sesui-muted dark:text-sesui-dmuted">{it.aciklama}</span>
            </span>
            <span className="text-[16px] font-bold tabular-nums text-sesui-text dark:text-sesui-dtext">{it.deger}</span>
            {it.l && <Rozet l={it.l} />}
          </li>
        ))}
      </ul>
      {!compact && <p className="text-[14px] text-sesui-body dark:text-sesui-dbody mt-2">{acousticComment(ac)}</p>}
      {!compact && (
        <p className="text-[13px] text-sesui-muted dark:text-sesui-dmuted mt-1">
