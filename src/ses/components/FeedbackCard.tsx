  ...
  return (
    <section className="ses-card space-y-3">
      <div className="flex items-center gap-3">
        <span className="w-12 h-12 rounded-2xl bg-ses-50 dark:bg-[#352820] grid place-items-center text-[26px]">🤖</span>
        <div className="flex-1">
          <h3 className="text-[19px] font-bold text-slate-900 dark:text-[#f5ece4] leading-tight">Yapay zeka geri bildirimi</h3>
          <p className="text-[15px] text-slate-700 dark:text-[#d8c8bf]">Sonuçlar ve son 15 seansın eğilimi Claude'a gönderilir; ses kaydı gönderilmez. Seans başına birkaç sent.</p>
        </div>
      </div>
      {text ? (
        <div className="rounded-2xl bg-ses-50 dark:bg-[#352820] p-3 text-[17px] text-slate-800 dark:text-[#f5ece4] whitespace-pre-wrap leading-relaxed">{text}</div>
      ) : !keyOk ? (
      ...
      {text && plan && (plan.focus.length > 0 || plan.goal) && (
        <div className="rounded-2xl border border-ses-200 dark:border-[#4a3a30] p-3 space-y-2">
          ...
          {plan.goal && <p className="text-[17px] font-semibold text-slate-800 dark:text-[#f5ece4]">🎯 {plan.goal}</p>}
          ...
          <button
            className={`w-full min-h-[54px] rounded-2xl text-[17px] font-semibold ${applied ? 'bg-emerald-50 dark:bg-[#1f2e22] text-emerald-700 dark:text-emerald-300' : 'bg-ses-600 text-white'}`}
            onClick={() => {
              applyPlan(plan)
              setApplied(true)
            }}
      ...
      {err && <p className="text-[16px] text-rose-600">{err}</p>}
      {keyOk && (
