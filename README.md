...
| 🎬 **Videolar** | Uygulamaya gömülü **kısa gösterim klipleri** (`public/ses-video/<id>.mp4`, yapay zekâ ile üretilmiş, internetsiz oynar; şimdilik karın nefesi) egzersiz girişinde en üstte; ardından YouTube. Seans giriş kartının üstünde **gömülü video** ("Başlamadan önce izle": kapak resmi + ▶, dokununca uygulama içinde oynar; internet gerekir), altında "YouTube'da aç". Ana sayfa ve Egzersizler'den açılan "Videolar" sayfası tüm videoları listeler; videosu bulunan egzersizlerde (nefes, dudak trili, mırıldanma, VFE, sert başlangıç, yarım yutkunma, uzun "A", gür ses/PhoRTE, suda tüp, pipet, tekerlemeler, kalem, artikülasyon, vurgu/tonlama/hız) giriş kartında ve ayrıntı sayfasında "Videoyu izle" düğmesi; öksürükten sese, baş çevirme ve itme/çekme için birebir gösterim bulunamadığından "(Genel)" etiketli ses teli felci egzersiz videoları (KBB uzmanı, EN; Dr. Le Huche, FR) verildi; YouTube telefonun tarayıcısında açılır (Türkçe/İngilizce işaretli). Bağlantılar `src/ses/lib/content.ts` içindeki `VIDEOS` listesinde |
